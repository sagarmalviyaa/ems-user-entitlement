...
router.put('/user-role/:userId', (req, res) => {
  try {
    const userId = req.params.userId;
    const {roleTypeId, rolePlace} = req.body; // This should contain the updated user roles

    // Assuming 'user_roles' is your table name
    connection.query('UPDATE user_role SET role_type_id = ?, role_place = ? WHERE user_id = ?', [roleTypeId,rolePlace, userId], (error) => {
      if (error) {
